...
float4 main(float4 position : SV_Position, float2 uv : TEXCOORD0) : SV_Target
{
    ...
    float3 P0 = SourceTexture.Sample(PointSampler, uv + float2(-dx, -dy)).xyz;
    float3 P1 = SourceTexture.Sample(PointSampler, uv + float2(2.0 * dx, -dy)).xyz;
    float3 P2 = SourceTexture.Sample(PointSampler, uv + float2(-dx, 2.0 * dy)).xyz;
    float3 P3 = SourceTexture.Sample(PointSampler, uv + float2(2.0 * dx, 2.0 * dy)).xyz;

    float3 B = SourceTexture.Sample(PointSampler, uv + float2(0, -dy)).xyz;
    float3 C = SourceTexture.Sample(PointSampler, uv + float2(dx, -dy)).xyz;
    float3 H5 = SourceTexture.Sample(PointSampler, uv + float2(0, 2.0 * dy)).xyz;
    float3 I5 = SourceTexture.Sample(PointSampler, uv + float2(dx, 2.0 * dy)).xyz;

    float3 D = SourceTexture.Sample(PointSampler, uv + float2(-dx, 0)).xyz;
    float3 F4 = SourceTexture.Sample(PointSampler, uv + float2(2.0 * dx, 0)).xyz;
    float3 G = SourceTexture.Sample(PointSampler, uv + float2(-dx, dy)).xyz;
    float3 I4 = SourceTexture.Sample(PointSampler, uv + float2(2.0 * dx, dy)).xyz;

    float3 E = SourceTexture.Sample(PointSampler, uv).xyz;
    float3 F = SourceTexture.Sample(PointSampler, uv + float2(dx, 0)).xyz;
    float3 H = SourceTexture.Sample(PointSampler, uv + float2(0, dy)).xyz;
    float3 I = SourceTexture.Sample(PointSampler, uv + float2(dx, dy)).xyz;
