...
COMPLEX groups pass for all 22 commands, and since bug 78 so does its TIMING
group. Its OPCODE group does not: RTPS and RTPT fail on value, and NCS, NCT,
NCCS, NCCT, CC, CDP, NCDS, NCDT and MVMVA on flags (bug 78 has the list; not
yet investigated). The BIOS shell issues zero GTE
...
- **amidog's GTE suite has run** (`test/psxtest_gte/` - see bug 41 for how
  ...
  command). BASIC, REG, COMPLEX and, since bug 78, the official TIMING group
  pass - all 22 opcodes' T green, sampled from the screen at frame 60,000.
  The OPCODE group still shows value errors for RTPS and RTPT and flag errors
  for nine more commands (bug 78), so TOTAL is red for F and V.
